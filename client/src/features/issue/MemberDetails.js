...
function MemberDetails({ member, currentUser, memberId, update, setUpdate }) {
  const [id, member_email] = member;
  const defaultURL =
  ...
  useEffect(() => {
    fetch("/users")
      .then((res) => res.json())
      .then((data) => setUserInfo(data.find((user) => user.email === member)));
  }, []);

  function handleRemove(e) {
    e.stopPropagation();
    fetch(`/user_projects/${memberId}`, {
      method: "DELETE",
    }).then((r) => {
      if (r.ok) {
        console.log("Deleted");
      ...
      {!userInfo ? (
        <div className="member-card-inner">
          <ReactRoundedImage
            ...
            imageHeight="80"
          />
          <div className="member-card-details">{member}</div>
        </div>
      ) : (
        <div className="member-card-inner">
          {userInfo.picture ? (
